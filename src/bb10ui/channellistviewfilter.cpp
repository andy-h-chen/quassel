...
*****************************************/
ChannelListViewFilter::ChannelListViewFilter(QAbstractItemModel *model, BufferViewConfig *config)
...
{
    qDebug() << "xxxxx ChannelListViewFilter::ChannelListViewFilter begin";
    setConfig(config);
    ...
    qDebug() << "xxxxx ChannelListViewFilter::ChannelListViewFilter end";
}


void ChannelListViewFilter::setConfig(BufferViewConfig *config)
{
    if (_config == config)
        return;
...
void ChannelListViewFilter::configInitialized()
{
    if (!config())
        return;
...
void ChannelListViewFilter::showServerQueriesChanged()
{
...
QList<QAction *> ChannelListViewFilter::actions(const QModelIndex &index)
{
    Q_UNUSED(index)
...
void ChannelListViewFilter::enableEditMode(bool enable)
{
    if (_editMode == enable) {
        return;
    }
...
Qt::ItemFlags ChannelListViewFilter::flags(const QModelIndex &index) const
{
...
bool ChannelListViewFilter::dropMimeData(const QMimeData *data, Qt::DropAction action, int row, int column, const QModelIndex &parent)
{
...
void ChannelListViewFilter::sort(int column, Qt::SortOrder order)
{
    _sortOrder = order;
...
void ChannelListViewFilter::addBuffer(const BufferId &bufferId) const
{
...
void ChannelListViewFilter::addBuffers(const QList<BufferId> &bufferIds) const
{
    if (!config())
        return;
...
bool ChannelListViewFilter::filterAcceptBuffer(const QModelIndex &source_bufferIndex) const
{
    ...
    if (!config()->bufferList().contains(bufferId) && !_editMode) {
        ...
        qDebug() << "xxxxx ChannelListViewFilter::filterAcceptBuffe line 346";
        return false;
    }
    ...
    if (config()->networkId().isValid() && config()->networkId() != sourceModel()->data(source_bufferIndex, NetworkModel::NetworkIdRole).value<NetworkId>()) {
        qDebug() << "xxxxx ChannelListViewFilter::filterAcceptBuffe line 351";
        return false;
    }
    ...
    if (!(allowedBufferTypes & bufferType)) {
        qDebug() << "xxxxx ChannelListViewFilter::filterAcceptBuffe line 360";
        return false;
    }
    ...
    if (bufferType & BufferInfo::QueryBuffer && !_showServerQueries && sourceModel()->data(source_bufferIndex, Qt::DisplayRole).toString().contains('.')) {
        qDebug() << "xxxxx ChannelListViewFilter::filterAcceptBuffe line 365";
        return false;
    }
...
bool ChannelListViewFilter::filterAcceptNetwork(const QModelIndex &source_index) const
{
    if (!config())
        return true;
...
bool ChannelListViewFilter::filterAcceptsRow(int source_row, const QModelIndex &source_parent) const
{
    return true;
    // FIXME: need more investigation to make this code to work
    /*
    ...
    qDebug() << "xxxx ChannelListViewFilter::filterAcceptsRow childType = " << childType;
    switch (childType) {
...
bool ChannelListViewFilter::lessThan(const QModelIndex &source_left, const QModelIndex &source_right) const
{
...
bool ChannelListViewFilter::bufferLessThan(const QModelIndex &source_left, const QModelIndex &source_right) const
{
...
bool ChannelListViewFilter::networkLessThan(const QModelIndex &source_left, const QModelIndex &source_right) const
{
...
QVariant ChannelListViewFilter::data(const QModelIndex &index, int role) const
{
    switch (role) {
    case Qt::FontRole:
    ...
    case Qt::DecorationRole:
        if ((config() && config()->disableDecoration()))
            return QVariant();
        return Bb10Ui::uiStyle()->channelListViewItemData(mapToSource(index), role);
...
QVariant ChannelListViewFilter::checkedState(const QModelIndex &index) const
{
    if (!_editMode || !config())
        return QVariant();
...
bool ChannelListViewFilter::setData(const QModelIndex &index, const QVariant &value, int role)
{
    switch (role) {
    case Qt::CheckStateRole:
...
bool ChannelListViewFilter::setCheckedState(const QModelIndex &index, Qt::CheckState state)
{
    QModelIndex source_index = mapToSource(index);
    BufferId bufferId = sourceModel()->data(source_index, NetworkModel::BufferIdRole).value<BufferId>();
    if (!bufferId.isValid())
...
void ChannelListViewFilter::checkPreviousCurrentForRemoval(const QModelIndex &current, const QModelIndex &previous)
{
    Q_UNUSED(current);
    if (previous.isValid())
...
void ChannelListViewFilter::customEvent(QEvent *event)
{
...
void ChannelListViewFilter::checkItemsForRemoval(const QModelIndex &topLeft, const QModelIndex &bottomRight)
{
...
bool ChannelListViewFilter::bufferIdLessThan(const BufferId &left, const BufferId &right)
{
